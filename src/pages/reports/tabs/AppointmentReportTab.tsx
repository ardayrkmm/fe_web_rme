...
export default function AppointmentReportTab() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['report-appointments', patientId, physiotherapistId, startDate, endDate],
  ...
  const handleExportPDF = () => {
    const columns = ['Tanggal', 'Waktu', 'Pasien', 'Fisioterapis', 'Status'];
    ...
    exportToPDF('Laporan Janji Terapi', columns, rowData);
  };

  const handleExportExcel = () => {
    const rowData = records.map((r: any) => ({
      'Tanggal': r.appointment_date ? new Date(r.appointment_date).toLocaleDateString() : '-',
      'Waktu': r.appointment_time,
      'Pasien': r.patient?.name || '-',
      'Fisioterapis': r.physiotherapist?.name || '-',
      'Status': r.status
    }));
    exportToExcel('Laporan_Janji_Terapi', rowData);
  };
  ...
  const handlePrint = () => {
    ...
    printReport('Laporan Janji Terapi', columns, rowData);
  };

  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle>Laporan Janji Terapi</CardTitle>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                records.map((r: any) => (
                  <TableRow key={r.id}>
                    <TableCell>{r.appointment_date ? new Date(r.appointment_date).toLocaleDateString() : '-'}</TableCell>
                    <TableCell>{r.appointment_time}</TableCell>
